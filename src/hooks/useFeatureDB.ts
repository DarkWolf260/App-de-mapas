  ...
  const handleSaveDailyLog = async (featureId: number, log: DailyLog): Promise<void> => {
    try {
      ...
      const payload = {
        ...
        officers_count: log.officersCount || "",
        has_arrived_g1: !!log.hasArrivedG1,
        commission_id: log.commissionId || "independiente",
        is_volunteer: !!log.isVolunteer,
        group_name2: log.groupName2 || "",
        unit_out2: log.unitOut2 || "",
        manager_name2: log.managerName2 || "",
        manager_phone2: log.managerPhone2 || "",
        ...
        commission_id3: log.commissionId3 || "independiente",
        is_volunteer3: !!log.isVolunteer3,
        ...
        commission_id4: log.commissionId4 || "independiente",
        is_volunteer4: !!log.isVolunteer4,
        rescued_count: groupsList[0]?.rescuedCount ?? (log.rescuedCount || ""),
        recovered_count: groupsList[0]?.recoveredCount ?? (log.recoveredCount || ""),
        rescued_pets_count: groupsList[0]?.rescuedPetsCount ?? (log.rescuedPetsCount || ""),
        prehospital_care_count: groupsList[0]?.prehospitalCareCount ?? (log.prehospitalCareCount || ""),
        transfers_count: groupsList[0]?.transfersCount ?? (log.transfersCount || ""),
        observations: log.observations || "",
        updated_at: new Date().toISOString(),
      };
